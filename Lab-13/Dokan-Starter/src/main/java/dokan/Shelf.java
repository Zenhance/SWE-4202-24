...
public final class Shelf<T extends Item> {
    ...
    public boolean contains(String name) {
        for(int i=0;i<items.size();i++) if(Objects.equals(items.get(i).name(), name)) return true;
        return false;
    }

    /** Takes the first item with this name off the shelf. True if one was there. */
    public boolean remove(String name) {
        for(int i=0;i<items.size();i++) if(Objects.equals(items.get(i).name(), name)) {items.remove(i);return true;}
        return false;
    }
