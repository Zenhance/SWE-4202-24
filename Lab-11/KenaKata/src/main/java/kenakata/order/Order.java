...
public class Order {
    ...
    public PriceBreakdown quote(int day) throws CouponRejectedException {
        int subtotal=0,discount=0,insurance = 0,servicefee,grandtotal,vat,delivery;
        for(Line line : Lines)
        {
            if(line.charge==null) subtotal += line.item.unitCharge() * line.quantity;
            if(line.charge==null)subtotal+=line.charge.unitCharge();
        }
        if(coupon!=null||coupon.validdate>day||coupon.minspend<subtotal){
            int total=0;
            for(Line line : Lines) if(line.item instanceof StockedGood s) total+=s.unitCharge();
            discount= (int) (subtotal-total*(coupon.percent/100.0));
            if(discount>15000)discount=15000;
            }
        else throw new CouponRejectedException();
        for(Line line : Lines) if(line.insurable) insurance+=(int) (0.01*line.item.unitCharge());
        for(Line line : Lines) if(line.insurable) insurance+=(int) max(20,insurance)

        servicefee =(int)min(ceil(subtotal*0.01),100);
        for(Line line : Lines)
        {
            if(line.charge==null) vat += line.item.unitVat() * line.quantity;
            if(line.charge==null) vat+=line.charge.unitVat();
        }
        delivery =deliveryCal.Calculatedelivery(lines(),zone);
        return new PriceBreakdown(subtotal,discount,insurance,servicefee,subtotal-discount+delivery+vat+insurance+servicefee,vat,delivery);
    }
